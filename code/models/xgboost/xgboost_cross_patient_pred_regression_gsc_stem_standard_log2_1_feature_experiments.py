...
def get_data_patient_1(file_path, indices, 
                       gene_dict, num_genes, 
    ...
    if preprocess:
        ...
        for name in tqdm(gene_names):
            # Each subset is of shape 100 x 6 (number
            # of 100bp bins x number of columns)
...
def get_data_patient_2(file_path, indices, 
    ...
    if preprocess:
        ...
        #ind = np.load('/gpfs/data/rsingh47/Tapinos_Data/Realigned_data_files/ind_shuffle_for_Omnibus_datasets.npy', allow_pickle=True)
        # 11/1/24 Custom shuffle index for Omnibus v3 datasets loaded
        #ind = np.load('/gpfs/data/rsingh47/Tapinos_Data/Realigned_data_files/ind_shuffle_for_Omnibus_v3_datasets.npy', allow_pickle=True)        
        ...
        for dataset in datasets:
            ...
            # 10/28/24 No standardization on the last 3 features of GSC's with proxy
            # features set to 0. Avoid divide by 0 error.
...
def train_model(X_train, X_val, Y_train, Y_val, validation, 
                ...
                min_child_weight, colsample_bytree, subsample, gamma, count):
    """
    Implements and trains a XGBoost model making use of only 1 feature
    ...
    #11/5/24 Use only H3K27Ac for training and testing
    X_train = X_train[:,:,0]
    print(X_train.shape)
...
def test_model(model, X_test, Y_test, learning_rates, 
               n_estimators, max_depths, min_child_weight, 
               colsample_bytree, subsample, gamma, count):
    """
    Evaluates the trained XGBoost model 
    making use of only 1 feature
...
def prediction_csv(se_value, y_true, y_pred, gene_names):
    ...
    predicted RNAseq values along
    with the calculation of the Squared Error 
    into a csv file saved to the 
    model's images directory.
    
    param se_value: Array of the calculated 
                    Squared Error values per gene.
    param y_true: the true (observed) RNAseq values.
    param y_pred: the model's predicted values.
   ...
   # Create csv file to hold prediction information.
    ...
    for i in tqdm(range(len(gene_names))):
        ...
        tv = y_true[i]
        pv = y_pred[i]
        with open(save_directory + '/xgboost_cross_patient_regression_gsc_stem_standard_test_predictions.csv', 'a') \
        as log:
            log.write('\n' f'{gn}, {tv[0]}, {pv}, {se}')
...
def visualize_training_validation_distributions(y_train, y_val):
    ...
    plt.savefig(save_directory + 
                '/Training_set_genes_RNAseq_value_counts-_histogram_plot.png', 
                bbox_inches='tight')
    #plt.show()
    ...
    training_expression_counts_dataframe = pd.DataFrame({"expression catagory after log(2) transformation" : 
                                                         ["all zero", 
                                                          ...
                                                          "between 10 and 15" ], 
                                                         "count": 
                                                         [len(training_all_zero_true_expression),
    ...
    validation_expression_counts_dataframe = pd.DataFrame({"expression catagory after log(2) transformation" : 
                                                           ["all zero", 
                                                            ...
                                                            "between 10 and 15" ],
                                                           "count": 
                                                           [len(validation_all_zero_true_expression),
...
def visualize_model_test_results(result_1, result_2, result_3):
    ...
    sn.set_theme(style = 'whitegrid')
    fig, ax = plt.subplots(figsize = (8, 5))
    ax = sn.barplot(data = results_dataframe, 
                    x = "metric", y = "test set results")
...
def load_csv_and_create_dataframes():
    ...
    prediction_dataframe = pd.read_csv(save_directory + 
                                       '/xgboost_cross_patient_regression_gsc_stem_standard_test_predictions.csv', 
...
def visualize_test_obs_pred(y_true, y_pred):
    ...
    data = {'True Values': np.squeeze(y_true), 
            'Predicted Values': np.squeeze(y_pred)}
...
def main(loss_dict, pcc_dict, r2_score_dict, scc_dict, val_loss_dict, val_pcc_dict, val_r2_score_dict, val_scc_dict, gene_dict, num_genes, count, learning_rates, n_estimators, max_depths, min_child_weight, colsample_bytree, subsample, gamma):
    ...
    visualize_feature_importances_sums(h3k27ac_mean_importances, 
                                       atac_mean_importances, 
                                       ctcf_mean_importances,
                                       rnapii_mean_importances, 
                                       h3k27ac_sum_importances, 
                                       atac_sum_importances,
                                       ctcf_sum_importances, 
                                       rnapii_sum_importances)

    visualize_feature_importances_means(h3k27ac_mean_importances, 
                                        atac_mean_importances, 
                                        ctcf_mean_importances,
                                        rnapii_mean_importances, 
                                        h3k27ac_sum_importances, 
    ...
    gene_names_in_test_set = get_gene_names(gene_dict, 
                                            indices, 
                                            X_test.shape[0], 
                                            num_genes, 
                                            test_set_indices)
    ...
    Y_pred = make_prediction(model, 
                             X_test)    
    prediction_se = calculate_se_for_predictions(Y_test, 
                                                 Y_pred)    
    prediction_csv(prediction_se, 
                   Y_test, 
                   Y_pred, 
                   gene_names_in_test_set)    
    visualize_training_validation_distributions(Y_train, 
                                                Y_val)    
    visualize_model_test_results(test_PCC, 
                                 test_SCC, 
                                 test_R2)
    visualize_se_heatmap(prediction_se, 
                         gene_names_in_test_set)
    ...
    visualize_testing_distributions(all_zero_true_expression, 
                                    true_expression_between_0_and_5, 
                                    true_expression_between_5_and_10, 
                                    true_expression_between_10_and_15)
    ...
    visualize_testing_set_mse_by_catagory(test_set_MSE, 
                                          all_zero_true_expression, 
                                          true_expression_between_0_and_5, 
                                          true_expression_between_5_and_10, 
                                          true_expression_between_10_and_15)
    visualize_prediction_mse(prediction_se, 
                             Y_test, 
                             Y_pred)    
    visualize_test_obs_pred(Y_test, 
                            Y_pred)
    visualize_aggregated_input_profiles(X_test, 
                                        all_zero_true_expression, 
                                        true_expression_between_0_and_5, 
                                        true_expression_between_5_and_10,
                                        true_expression_between_10_and_15,prediction_dataframe)    
