'''
This is a linear regression model composed using the Statsmodels 
api and configured for cross-patient prediction. 

It takes two patient datafiles in numpy format. The model 
is trained and validated on the first position datafile.
The model is then tested on the second position file. The third input position
is designated for the index file (composed as a numpy array) that is used 
to shuffle and allocate the data for cross-validation.
'''
import sys
import numpy as np
import os
...
def train_model(X_train, X_val, Y_train, Y_val, validation, alpha, L1_wt):
    """
    Implements and trains a SVM model.
    ...
    #### NOTE this setup was not used here. The data is reshaped below.
    # save train and test data into variables
    #x_train = sm.add_constant(np.vstack([train[0], val[0]]))
    #y_train = np.hstack([train[1], val[1]])
    #x_test = sm.add_constant(test[0])
    #y_test = test[1]


    # UN-COMMENT TO PRINT FEATURE WEIGHTS
    #print(est.summary())
    ...
    #Code to reshape data into 2 dimensions. This is the same strategy used in the 
    # GBR and SVR models for consistency.
    #reshaped_X_train = sm.add_constant(X_train.reshape((X_train.shape[0], -1), order = 'F'))
    ...
    # A method of reshaping the data 
    ...
    est = sm.OLS(reshaped_Y_train, reshaped_X_train).fit_regularized(method='elastic_net', alpha = alpha, L1_wt = L1_wt)
    if validation == True:
...
def main(loss_dict, pcc_dict, r2_score_dict, scc_dict, val_loss_dict, val_pcc_dict, val_r2_score_dict, val_scc_dict, gene_dict, num_genes, count, alpha, L1_wt):
    ...
    if validation_bool == True:
        model, PCC, SCC, R2  = train_model(X_train, X_val, Y_train, Y_val, validation=validation_bool, alpha = alpha, L1_wt = L1_wt)
        ...
        model  = train_model(X_train, X_val, Y_train, Y_val, validation=validation_bool, alpha = alpha, L1_wt = L1_wt)
    ...
    with open('pngs_mlr_cross_patient_pred_regression_gsc_stem_standard_log2/mlr_cross_patient_regression_gsc_stem_standard_log2_info.csv', 'a') as log:
