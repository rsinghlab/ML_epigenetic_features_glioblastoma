"""
Goal: Predict gene expression value (regression) from epigenetic signal inputs across two patients. 

The model takes two GSC (patient) data files as input. The first is used to create train and validation sets. The second, to create the test set. A third input is the reference genome (HG38) data.
 
This model is designed to accept 'raw' input values for the epigenetic features. It applies a log(2) transformation on the target variable before the cross-validation dataset split step.

The model applies standardization to the train, validation and test datasets seperately after the cross_validation split.  

The Reference Human Genome (HG38) data file is derived by first converting the reference genome to a numerical representation where each nucleotide is given a representative number. Within this script's training and testing processed, the numerical representations are one-hot encoded for model input.
"""
...
def get_data_patient_1(file_path_1, file_path_3, indices, gene_dict, num_genes, preprocess, validation):
    ...
    if preprocess:
        ...
        # Save sequence data.
        np.save("X_Sequence_cross_patient_regression_patient_1_stem_standard_log2_train", X_Seq_train, allow_pickle=True)
        np.save("X_Sequence_cross_patient_regression_patient_1_stem_standard_log2_val", X_Seq_val, allow_pickle=True)
        ...
        # Load sequence data.
        X_Seq_train = np.load("X_Sequence_cross_patient_regression_patient_1_stem_standard_log2_train.npy", allow_pickle=True)
...
def get_data_patient_2(file_path_2, file_path_3, indices, gene_dict, num_genes, preprocess):
    '''
    ...
    if preprocess:
        ...
        # Save sequence data.
        np.save("X_Sequence_cross_patient_regression_patient_1_stem_standard_log2_test", X_Seq_test, allow_pickle=True)
        ...
        # Load sequence data.
        X_Seq_test = np.load("X_Sequence_cross_patient_regression_patient_1_stem_standard_log2_test.npy", allow_pickle=True)
        
        gene_dict = gene_dict
        num_genes = num_genes

    return X_test, Y_test, gene_dict, num_genes, X_Seq_test
...
def train_model(X_train, X_val, Y_train, Y_val, X_Seq_train, X_Seq_val, epochs, validation, batch_size, learning_rate, epigenetic_branch_hidden_size_1, epigenetic_branch_hidden_size_2, epigenetic_branch_hidden_size_3, epigenetic_branch_dropout_rate, sequence_branch_hidden_size_1, sequence_branch_hidden_size_2, sequence_branch_hidden_size_3, sequence_branch_dropout_rate, combined_hidden_size_1, combined_hidden_size_2, combined_hidden_size_3, combined_dropout_rate):
    ...
    # Plot model graph
    plot_model(model, to_file = 'pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/mlp_cross_patient_pred_branched_regression_model.png', show_shapes=True)
...
def visualize_loss(history, validation, count):
    ...
    plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/mlp_loss_' + str(count) + '.png')
    #plt.show()
...
def visualize_pcc(history, validation, count):
    ...
    plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/mlp_pcc_' + str(count) + '.png')
    #plt.show()
...
def visualize_scc(history, validation, count):
    ...
    plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/mlp_scc_' + str(count) + '.png')
    #plt.show()
...
def visualize_rsquare(history, validation, count):
    ...
    plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/mlp_rsquare_' + str(count) + '.png')
    #plt.show()
...
def visualize_metrics_together(history, validation, count):
    ...
    plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/mlp_metrics_' + str(count) + '.png')
    #plt.show()
...
def visualize_training_validation_distributions(y_train, y_val, validation):
    ...
    plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/Training_set_genes_RNAseq_value_counts-_histogram_plot.png')
    #plt.show()
    ...
    for l in range(len(dataframes)):
        ...
        for i in ax.containers:
            ax.bar_label(i,)
        plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/Cross_patient_Regression_' + dataset_names[l] + '_Expression_Catagory_Counts.png', bbox_inches='tight')
...
def visualize_prediction_mse(prediction_mses, y_true, y_pred):
    ...
    plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/MLP_regression_test_set_mse_values.png', bbox_inches='tight')
    #plt.show()
    ...
    plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/MLP_regression_test_set_mse_values_-_histogram_plot.png', bbox_inches='tight')
    #plt.show()
    ...
    plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/MLP_regression_test_set_true_RNAseq_values_-_histogram_plot.png', bbox_inches='tight')
    #plt.show()
    ...
    plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/MLP_regression_test_set_predicted_RNAseq_values_-_histogram_plot.png', bbox_inches='tight')
    #plt.show()
    return None
...
def visualize_prediction(highly_expressed_low_mse_group, lowly_expressed_low_mse_group, highly_expressed_high_mse_group, lowly_expressed_high_mse_group, mse_value, test_true_values, test_prediction, gene_names):
    ...
    def prediction_plot(gene_indices, chart_label, mse_value, true_RNAseq_value, predicted_RNAseq_value, gene_names):
        ...
        fig.tight_layout()
        plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/MLP_regression_Prediction_' + chart_label + '.png')
        #plt.show()
...
def visualize_test_obs_pred(y_true, y_pred):
    ...
    plt.xlim(0, 15)
    plt.xlim(0, 15)
    plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/MLP_regression_test_set_observed_vs_predicted.png', bbox_inches='tight')
    #plt.show()
    ...
    plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/MLP_regression_test_set_observed_vs_predicted_joint_plot.png', bbox_inches='tight')
    #plt.show()
    ...
    plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/MLP_regression_test_set_observed_vs_predicted_KDE_joint_plot.png', bbox_inches='tight')
    #plt.show()
...
def visualize_aggregated_input_profiles(test_dataset, all_zero_true_expression, true_expression_between_0_and_5, true_expression_between_5_and_10, true_expression_between_10_and_15, prediction_dataframe):
    ...
    for h in tqdm(range(len(heatmap_indexes))):
        ...
        plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/' + heatmap_names[h] + '_seaborn.png', dpi = 150, bbox_inches = 'tight')
        #plt.show()
...
def visualize_correlation(test_data, highly_expressed_low_mse_group, lowly_expessed_low_mse_group, highly_expressed_high_mse_group, lowly_expressed_high_mse_group):
    ...
    for h in range(len(correlation_indexes)):
        ...
        plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/MLP_' + correlation_names[h] +'.png')
        #plt.show()
        ...
        plt.title(correlation_names[h])
        plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/MLP_' + correlation_names[h] +'_diagonal_plot.png')
        #plt.show()
...
def visualize_shap_analysis(deep_shap_values, gene_name_list, genes_of_interest_list, visualize_full_dataset_results = False):
    ...
    for gene in tqdm(genes_of_interest):
        ...
        plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/shap_plots/' + gene + '_-_regression_SHAP_summary_plot - deep_explainer_dot_plot.png', dpi = 100, bbox_inches = 'tight')
        #plt.show()
        ...
        ####plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/shap_plots/' + gene + '_-_regional_analysis_-_regression_SHAP_summary_plot - deep_explainer_dot_plot.png')
        #plt.show()
        ...
        plt.title(f"SHAP summary for gene test index {index} : {gene}")
        plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/shap_plots/' + gene + '_-_cross_patient_regression_SHAP_deep_explianer_bar_plot.png', dpi = 100, bbox_inches = 'tight')
        #plt.show()
        ...
        ####plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/shap_plots/' + gene + '_-_regional_analysis_-_regression_SHAP_summary_plot - deep_explianer_bar_plot.png')
        #plt.show()

        #plt.close()
        #shap.summary_plot(gradient_shap_values[0], plot_type = 'bar', show=True)
        #plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/shap_plots/Regression_SHAP_summary_plot - gradient.png')

        #plt.close()
        #shap.plots.bar(deep_shap_values)
        #plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/shap_plots/Regression_SHAP_summary_plot - gradient-bar.png')

        # Summary plot 2
        #plt.close()
        #shap.plots.beeswarm(deep_explainer, show = False)    
        #plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/shap_plots/Regression_SHAP_beeswarm_plot.png')
        ...
        ####plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/shap_plots/' + gene + '_-_regression_SHAP_waterfall_plot.png')
        #plt.show()
        ...
        #plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/shap_plots/' + gene + '_regression_SHAP_force_plot.png')
        #plt.show()
        ...
        ####plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/shap_plots/'+ gene + '_-_regional_analysis_-_regression_SHAP_decision_plot.png')
...
def visualize_aggregated_shap_analysis(shap_values, deep_explainer, all_zero_true_expression, true_expression_between_0_and_5, true_expression_between_5_and_10, true_expression_between_10_and_15, prediction_dataframe):
    ...
    for i in tqdm(range(len(gene_indecies))):
        ...
        plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/shap_plots/' + indices_names[i] + '_-_regression_SHAP_summary_plot_-_deep_explainer_dot_plot.png', dpi = 100, bbox_inches = 'tight')
        #plt.show()
        ...
        ####plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/shap_plots/' + indices_names[i] + '_-_regional_analysis_-_regression_SHAP_summary_plot - deep_explainer_dot_plot.png')
        #plt.show()
        ...
        plt.title(f"{indices_names[i]}")
        plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/shap_plots/' + indices_names[i] + '_-_regression_SHAP_summary_plot_-_deep_explianer_bar_plot.png', dpi = 100, bbox_inches = 'tight')
        #plt.show()
        ...
        ####plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/shap_plots/' + indices_names[i] + '_-_regional_analysis_-_regression_SHAP_summary_plot - deep_explianer_bar_plot.png')
        #plt.show()
        ...
        #plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/shap_plots/' + i + '_regression_SHAP_waterfall_plot.png')
        #plt.show()
        ...
        ####plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/shap_plots/'+ indices_names[i] + '_-_regional_analysis_-_regression_SHAP_decision_plot.png')
...
def prediction_csv(se_value, y_true, y_pred, gene_names):
    ...
    '''
   # Create csv file to hold prediction information.
    with open('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/mlp_cross_patient_regression_gsc_stem_standard_test_predictions.csv', 'w') as log:
    ...
    for i in tqdm(range(len(gene_names))):
        ...
        tv = y_true[i]
        pv = y_pred[i]
        with open('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/mlp_cross_patient_regression_gsc_stem_standard_test_predictions.csv', 'a') as log:
            log.write('\n' f'{gn}, {tv[0]}, {pv[0]}, {se[0]}')
...
def load_csv_and_create_dataframes():
    ...
    prediction_dataframe = pd.read_csv('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/mlp_cross_patient_regression_gsc_stem_standard_test_predictions.csv', float_precision='round_trip')
...
def visualize_model_test_results(results_list):
    ...
    for i in ax.containers:
        ax.bar_label(i,)
    plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/mlp_cross_patient_regression_gsc_stem_standard_test_metric_results.png', bbox_inches='tight')
    return None
...
def visualize_se_heatmap(se_values, gene_names_in_test_set):
    ...
    plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/mlp_cross_patient_regression_gsc_stem_standard_test_gene_squared_error_heatmap.png', bbox_inches='tight')
    #plt.show()
...
def visualize_testing_distributions(all_zero_true_expression, true_expression_between_0_and_5, true_expression_between_5_and_10, true_expression_between_10_and_15):
    ...
    plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/mlp_cross_patient_regression_gsc_stem_standard_test_expression_catagory_counts.png', bbox_inches='tight')
...
def visualize_testing_set_mse_by_catagory(test_set_MSE, all_zero_true_expression, true_expression_between_0_and_5, true_expression_between_5_and_10, true_expression_between_10_and_15):
    ...
    plt.savefig('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/mlp_cross_patient_regression_gsc_stem_standard_test_expression_catagory_MSE.png', bbox_inches='tight')
...
def main(loss_dict, pcc_dict, r2_score_dict, scc_dict, val_loss_dict, val_pcc_dict, val_r2_score_dict, val_scc_dict, gene_dict, num_genes, count, batch_size, learning_rate, epigenetic_branch_hidden_size_1, epigenetic_branch_hidden_size_2, epigenetic_branch_hidden_size_3, epigenetic_branch_dropout_rate, sequence_branch_hidden_size_1, sequence_branch_hidden_size_2, sequence_branch_hidden_size_3, sequence_branch_dropout_rate, combined_hidden_size_1, combined_hidden_size_2, combined_hidden_size_3, combined_dropout_rate):
    ...
    # file_path_2 is the datafile for testing the model.
    ...
    #file_path_2 = "/gpfs/data/rsingh47/Tapinos_Data/Perturbation_analysis_count_and_data_files/H3K27ac_ALL_ZERO_--Realigned_count_files_GSC2_Stem_old_ATAC_process_with_featurecounts_RNAseq_entire_gene/raw/H3K27ac_ALL_ZERO_--gsc2_stem_old_ATAC_process_with_featurecounts_RNAseq_entire_gene.npy"

    # ALL 0 H3K27ac
    #file_path_2 = "/gpfs/data/rsingh47/Tapinos_Data/Perturbation_analysis_count_and_data_files/H3K27ac_ALL_ZERO_--Realigned_count_files_GSC2_Stem_old_ATAC_process_with_featurecounts_RNAseq_entire_gene/raw/H3K27ac_ALL_ZERO_--gsc2_stem_old_ATAC_process_with_featurecounts_RNAseq_entire_gene.npy"
    
    # ALL 0 RNAPolII
    #file_path_2 = "/gpfs/data/rsingh47/Tapinos_Data/Perturbation_analysis_count_and_data_files/RNAPol2_ALL_ZERO_--Realigned_count_files_GSC2_Stem_old_ATAC_process_with_featurecounts_RNAseq_entire_gene/raw/RNAPol2_ALL_ZERO_--gsc2_stem_old_ATAC_process_with_featurecounts_RNAseq_entire_gene.npy"

    # ALL 0 ATAC
    #file_path_2 = "/gpfs/data/rsingh47/Tapinos_Data/Perturbation_analysis_count_and_data_files/ATAC_ALL_ZERO_--Realigned_count_files_GSC2_Stem_old_ATAC_process_with_featurecounts_RNAseq_entire_gene/raw/ATAC_ALL_ZERO_--gsc2_stem_old_ATAC_process_with_featurecounts_RNAseq_entire_gene.npy"
    #file_path_2 = "../out_GSC2_ATAC_zero.npy"

    # All NEGATIVE CTCF
    #file_path_2 = '/gpfs/data/rsingh47/Tapinos_Data/Perturbation_analysis_count_and_data_files/CTCF_ALL_NEGATIVE_--Realigned_count_files_GSC2_Stem_old_ATAC_process_with_featurecounts_RNAseq_entire_gene/raw/CTCF_ALL_NEGATIVE_--gsc2_stem_old_ATAC_process_with_featurecounts_RNAseq_entire_gene.npy'

    #file_path_3 = "/gpfs/data/rsingh47/Tapinos_Data/HG38_reference_genome_data_files/HG38_sequence_input_numerical_encoding.npy"
    ...
    X_train, X_val, Y_train, Y_val, gene_dict, num_genes, X_Seq_train, X_Seq_val = get_data_patient_1(file_path_1, file_path_3, indices, gene_dict, num_genes, preprocess = preprocess_bool, validation = validation_bool)
    
    # Processing data for patient 2 file to produce test set.
    X_test, Y_test, gene_dict, num_genes, X_Seq_test = get_data_patient_2(file_path_2, file_path_3, indices, gene_dict, num_genes, preprocess = preprocess_bool)
    
    # Call train_model() to train the model
    print("Training model...")
    model, history = train_model(X_train, X_val, Y_train, Y_val, X_Seq_train, X_Seq_val, epochs=150, validation = validation_bool, batch_size = batch_size, learning_rate = learning_rate, epigenetic_branch_hidden_size_1 = epigenetic_branch_hidden_size_1, epigenetic_branch_hidden_size_2 = epigenetic_branch_hidden_size_2, epigenetic_branch_hidden_size_3 = epigenetic_branch_hidden_size_3, epigenetic_branch_dropout_rate = epigenetic_branch_dropout_rate, 
                                ...
                                combined_hidden_size_1 = combined_hidden_size_1, combined_hidden_size_2 = combined_hidden_size_2, combined_hidden_size_3 = combined_hidden_size_3, combined_dropout_rate = combined_dropout_rate)
    ...
    with open('pngs_mlp_cross_patient_pred_regression_gsc_stem_sequence_standard_log2/mlp_cross_patient_regression_gsc_stem_sequence_standard_log2_info.csv', 'a') as log:
